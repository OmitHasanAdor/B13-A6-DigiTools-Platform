...
<div className="tabs tabs-box bg-white  rounded-full shadow-md max-w-50 mx-auto justify-center items-center">
  <input onClick={()=>setTabSelect('product')} type="radio" name="my_tabs_1" className={`tab  rounded-4xl  font-semibold ${tabSelect==='product' ? 'bg-linear-to-r from-[#4f39f6] to-[#9514fa] text-white' : ' text-black bg-white'}`} aria-label="Products" defaultChecked/>
  <input onClick={()=>setTabSelect('cart')} type="radio" name="my_tabs_1" className={`tab rounded-4xl  font-semibold ${tabSelect==='cart'? 'bg-linear-to-r from-[#4f39f6] to-[#9514fa] text-white': ' bg-white text-black'}`} aria-label={`Cart(${cart.length})`}  />
